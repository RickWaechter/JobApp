...
import DatabaseDownloader from '../inc/downloader.js';
import { prompt, loadModel } from './ModelFeature.js';
const OffApplication = () => {
...
const setDb = async () => {
      ...
      const prompt1 = `Schreibe eine ${selectedOption2 === 'Initiativ'
        ...
        : selectedOption2 === 'Regulär'
          ...
          : selectedOption2 === 'Praktikum'
            ...
            : 'Bewerbung'
        } Verfasse ein Bewerbungsschreiben als ${inputValue}. Ich habe ${erfahrung} Erfahrung.
	•	Keine Firmennamen oder spezifische Unternehmen nennen.
	•	Schreibe kein Betreff.
 ...
 `
      ...
      try {
        ...
        const results = await prompt(prompt1);
        console.log('MFResults', results);
        let newText = results.split("\n\n")
        newText = newText.splice(1)
        console.log('newText', newText);
        newText = newText.slice(0, -1);
        console.log('newText2', newText);
       if (newText[0].match("men und Herr")) {
        newText = newText.splice(1) 
        console.log('if' , newText);
       }
       if (newText[newText.length - 1].match("Name")) {
        newText = newText.slice(0, -1);
        console.log('if2' , newText);
       }
       if (newText[newText.length - 1].match("Grüßen")) {
        newText = newText.slice(0, -1);
        console.log('if2' , newText);
       
       }
       console.log('newText3', newText);
       newText.push("Mit freundlichen Grüßen")
  ...
  return (
    <SafeAreaView style={styles.safeContainer}>
      ...
      <View style={styles.container}>
        <Animated.View
          style={[
            {  transform: [{ translateY: Animated.multiply(keyboardHeight /2, -1) }]  }  // Diese Zeile fügt die dynamische Anpassung hinzu
          ]}
        >
          ...
          <View style={styles.section}>
          <DatabaseDownloader
                  downloadUrl="https://jobape.de/jobs.db"
                  dbName="jobs.db"
                  onReady={(ad) => {
                    console.log('DB ist bereit!', db);
                    
                  }}
                  alertTitle={t('download.titleJob')}
                  alertText={t('download.messageJob')}  
                  top={7}
                />
            <TextInput
              ref={jobRef}
