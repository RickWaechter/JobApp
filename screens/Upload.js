...
import { View, Text, Alert, StyleSheet, Pressable, TouchableOpacity, Dimensions, TouchableWithoutFeedback, Animated } from 'react-native';
...
import '../local/i18n';
import Sorter from '../comp/sorter.jsx'
  ...
  const handleFileChange = async () => {
    ...
    try {
      ...
      if (DocumentPicker.isCancel(err)) {
        ...
        console.error('Fehler beim Auswählen der Dateien:', err);
        setError('Fehler beim Auswählen der Dateien.');
      }
    }
  };
        ...
        <>
          ...
          >
            <View style={styles.entry}>
              <Card.Title style={styles.job}>{t('addAttachment')}</Card.Title>
              <Card.Divider />
          ...
          >
            <View style={styles.entry}>
              <Card.Title style={styles.job}>{t('replaceOldFiles')}</Card.Title>
              <Card.Divider />
                ...
                renderItem={({ item, drag, isActive }) => (
                  <View style={[styles.card, isActive && styles.cardActive]}>
                    <TouchableOpacity onPress={() => handleItemClick(item)} style={[styles.dragArea, isActive && styles.dragActive]} onLongPress={drag}>
                      <Text style={[styles.itemText, isActive && styles.itemTextActive]}>{item.name.substring(0, 30) + '...'}</Text>
                    </TouchableOpacity>
