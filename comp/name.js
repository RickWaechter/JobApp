  ...
  * ═══════════════════════════════════════════════════════════════════════ */
  ...
  const M = {
    ...
    inputH: 44,
    fieldGap: 12,       // Abstand zwischen den Feldern
  ...
  /* ── Höhen für die beiden Modi ──
    Such-Modus: Header + 1 Input + Liste + Button
    Manuell:    Header + 3 Inputs (+2 Abstände) + 2 Buttons */
  const H_SEARCH =
  ...
  const CompanySearchModal = ({
    ...
    const panelHeight = useMemo(() => {
      ...
      const needed = manualMode ? H_MANUAL : H_SEARCH;

      if (free <= needed) {
        return Math.round(Math.max(free, 240));
      }
      return Math.round(Math.min(needed + 60, free));
    }, [winH, stageTop, kbHeight, insets.bottom, manualMode]);
    ...
    const validateAndContinue = useCallback(() => {
      ...
      if (!yourName?.trim())
        next.nameErr = t("validation.nameCompany.required") || "Bitte Firmenname eingeben";
      if (!yourStreet?.trim())
    ...
    return (
      <View style={styles.root}>
        ...
        <Animated.View
          ...
          style={[styles.stage, { top: stageTop }]}
        >
          <Animated.View
            style={[
              styles.panel,
                ...
                <View style={styles.listCard}>
                  {results.length > 0 ? (
                    ...
                    <View style={styles.placeholderBox}>
                      {searching ? (
                        <ActivityIndicator size="small" color={WARM.iconLeading} />
                      ) : (
                        <MaterialIcons
                          name="travel-explore"
                          size={32}
                      ...
                      <Text style={styles.placeholderText}>
                        {query.length === 0
                          ...
                          : searching
                          ? t("companySearchModal.searching") || "Suche nach Orten..."
                          : t("companySearchModal.noResults") || "Keine Treffer gefunden"}
                      </Text>
                    </View>
                  )}
                </View>
