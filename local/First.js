...
import { ImageBackground, SafeAreaView, Animated, Dimensions, StyleSheet, View, Text, TextInput, TouchableOpacity, FlatList, Alert } from 'react-native';
...
const handleSuggestionClickStreet = (suggestion) => {
  ...
  const handleMyStreet = async value => {
    try {
      ...
      setYourStreet(value);
      const [res] = await db.executeSql(
       'SELECT rowid, street FROM streets WHERE street MATCH ? LIMIT 20',
        [`${value}*`],
      );
  ...
  useEffect(() => {
    ...
    async function dbExists(dbName) {
      const dbPath = RNFS.LibraryDirectoryPath +'/' + dbName;
    
      return await RNFS.exists(dbPath);
    }
    
    function openDbWithCopy(name) {
      return new Promise((resolve, reject) => {
        
        SQLite.openDatabase(
          {  name : name, createFromLocation : `~/${name}`},
          (db) => resolve(db),
          (err) => reject(err) 
        ); 
      });
    ...
    async function setupDatabases() {
      ...
      if (needsCopy) {
        console.log('Databases not found, copying from bundle...');
        for (const dbName of DB_NAMES) {
          await openDbWithCopy(dbName);
        }
        console.log('Databases copied from bundle.');
      } else {
