...
useEffect(() => {
  ...
  const initDatabase = async () => {
    ...
    try {
      ...
      // =========================================================
      // 🛠️ TEST-MODUS: Alte Datei vorab löschen
      // =========================================================
      const fileExistsBefore = await RNFS.exists(dest);
      if (fileExistsBefore) {
        await RNFS.unlink(dest);
        console.log('🧪 TEST: Alte jobs.db wurde gelöscht!');
      }
      // =========================================================
      ...
      if (!jobsDbExistsOnDisk) {
        ...
        // 1. Prüfen, ob der Server mit einem Erfolgsstatus (200-299) geantwortet hat
        if (!response.ok) {
          const errorText = await response.text();
          throw new Error(`API returned status ${response.status}: ${errorText.substring(0, 100)}`);
        }

        // 2. Antwort sicher parsen
        const data = await response.json();
        console.log("data", data);

        if (!data?.url) {
          throw new Error('Keine Download-URL in der API-Antwort gefunden');
        }
