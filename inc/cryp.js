...
import { sha256 } from 'react-native-sha256'; // sha256Bytes weggelassen, da nicht genutzt
...
export const encryptBase64 = async (base64String, iv, encryptionKey) => {
  try {
    ...
    // 1. SHA-256 Hash generieren (ist ein Hex-String)
    const keyString = await sha256(encryptionKey);

    // 2. FIX: Den String ZWINGEND in ein WordArray umwandeln!
    ...
    // Als reines Base64 ausgeben (ohne Salted__ Magic)
...
export const decryptBase = async (combinedString, encryptionKey) => {
  try {
    ...
    // Sicherheitscheck: Wenn leer, war der Key falsch (oder es ist ein altes PDF)
    if (!decryptedString) {
